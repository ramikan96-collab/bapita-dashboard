...
5. **`bookings_slot_unique` has no status predicate** — it is (business_id, appointment_date, appointment_time), so a *cancelled* row still blocks that slot forever. Expired deposit holds are therefore DELETED, not cancelled. Keep it that way. Since `2026-08-22-stay-business-type.sql` it is a **partial** index (`WHERE check_out IS NULL`) so it applies to appointments only — stays share a check-in time and would otherwise collide across units. Stay overlap is enforced in `src/lib/stay.ts`, not by the index.
