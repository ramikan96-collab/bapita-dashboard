...
# Batch 4 — UI/consistency sweep (Phase 3, last) — EXECUTION LOG (2026-07-05, Sonnet)

**Status: DONE.** App-code + `globals.css` only, no DB touch. `npm run lint` clean, `npm run build` clean (TypeScript + all 39 routes).

| # | Status | What shipped |
|---|--------|--------------|
| **#11 (186 hex → tokens)** | ✅ done, scoped | Added 4 new CSS custom properties to `globals.css` (`--color-danger`/`--color-danger-bg`/`--color-success`/`--color-success-bg`), values identical to what was already hardcoded everywhere (`#EF4444`/`#FEF2F2`/`#16A34A`/`#F0FDF4` — zero visual diff). Swept all 6 target literals (`#fff`, `#FFFFFF`, `#EF4444`, `#16A34A`, `#FEF2F2`, `#F0FDF4`) → `var(--color-surface)` / `var(--color-danger)` / `var(--color-success)` / their `-bg` pairs, across every dashboard page (`(dashboard)/**`), shared dashboard components (`AppShell`, `AddCustomerSheet`, `FontPicker`, `calendar/*`), and `login/page.tsx` — **162 occurrences across 27 files**, script-driven with an exact-value + word-boundary regex (verified no 8-digit alpha variants existed that a naive match could corrupt), diff spot-checked for correct CSS-color contexts (styles + SVG `fill`/`stroke` attrs, which do resolve `var()`). **Deliberately excluded from this sweep** (and left hardcoded on purpose, not missed): `src/app/api/**/route.ts` email-template HTML (7 files, ~55 literals) — CSS custom properties don't resolve in email clients, tokenizing these would break confirmation/notification emails; the public booking pages/themes under `src/app/[slug]/**` (customer-facing revenue path, each theme intentionally carries its own literal palette); third-party brand colors (Google/WhatsApp icon fills) — those are fixed brand identities, not app tokens; and the one-off decorative arrays (avatar palette, label chips) which are already centralized as local `const` objects per file — tokenizing single-use decorative swatches into global CSS vars adds indirection without reducing real debt. Remaining hex count in the untouched scope is a known, intentional carry-forward, not an oversight. |
| **#12 (spacing/alignment pass)** | ✅ surveyed, no new changes needed | Checked calendar (`calendar/page.tsx` + `components/calendar/*`), clients, settings, and new-booking for real (not just numeric-variety) inconsistencies: same-row button height mismatches, misaligned icon+text pairs, duplicated magic numbers that could drift. Border-radius spread across areas (7–12px small controls, 14–20px cards, 50%/999/9999 pills) is an intentional, coherent per-role scale, not drift — left as-is. No same-row height/alignment bugs found beyond what Batches 2–3 already fixed (clients toolbar 44px unification, new-booking CTA pairs already consistent at 48px). Nothing shipped here beyond the survey; flagging so it's not silently marked "done" when the honest result is "already fine." |

**Security phase, Batches 1–3, and Batches 2–4 (this audit's full scope) are now COMPLETE.** Manual-only carryover: **N7** (leaked-password protection toggle, Supabase dashboard) and a live anon-key probe of N1/N2 — both still require Rami, not scriptable via MCP/CLI.
