...
export function Pricing({ locale = "en" }: { locale?: Locale }) {
  ...
  // Domain is a recurring monthly charge like the four monthly ADDONS — it
  // stays in monthlyTotal regardless of site cadence, same as they do.
  const monthlyTotal =
    (yearly ? 0 : BASE_MONTHLY) + monthlyAddons.length * ADDON_PRICE + (domain ? DOMAIN_PRICE : 0);
    ...
    monthlyDisplayCount > 0
      ? yearly
        ? fill(
            ...
            { count: monthlyDisplayCount },
          )
        : fill(
            monthlyDisplayCount > 1
              ? t.monthlyDetailWithAddonsPlural
              : t.monthlyDetailWithAddons,
            { base: shekel(BASE_MONTHLY), count: monthlyDisplayCount },
          )
      : yearly
