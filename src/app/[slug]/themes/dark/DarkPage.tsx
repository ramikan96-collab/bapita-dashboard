...
export function DarkPage({ business, services }: Props) {
  ...
  const D = { bg: "#0D0D0D", surface: "#1A1A1A", raised: "#222222", text: "#F0F0F0", muted: "#888888", border: "rgba(255,255,255,0.08)" };

  return (
    <div dir={isRtl ? "rtl" : "ltr"} style={{ background: D.bg, minHeight: "100svh", color: D.text, fontFamily: "'Inter', system-ui, sans-serif" }}>
      <style>{`
      ...
      {/* Lang toggle */}
      <div style={{ position: "fixed", top: 16, right: 16, zIndex: 200, background: "rgba(255,255,255,0.1)", backdropFilter: "blur(8px)", borderRadius: 9999, padding: "4px", display: "flex", border: "1px solid rgba(255,255,255,0.15)" }}>
        {(["en", "he"] as Lang[]).map(l => (
          <button key={l} onClick={() => setLang(l)} style={{ background: lang === l ? "rgba(255,255,255,0.18)" : "none", border: "none", borderRadius: 9999, padding: "3px 11px", cursor: "pointer", fontSize: 12, fontWeight: lang === l ? 700 : 400, color: "#fff", fontFamily: "inherit", transition: "background 0.2s" }}>
            {l === "en" ? "EN" : "עב"}
          </button>
        ))}
      </div>
      ...
      <section style={{ position: "relative", height: "100svh", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, textAlign: "center", padding: "0 28px", width: "100%", maxWidth: 700 }}>
          <div className="dk-pill" style={{ marginBottom: 20, display: "flex", justifyContent: "center" }}>
            {openStatus ? (
              <span style={{ background: "rgba(255,255,255,0.1)", backdropFilter: "blur(8px)", color: "#fff", borderRadius: 9999, padding: "5px 14px", fontSize: 12, fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 6, border: "1px solid rgba(255,255,255,0.15)" }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: openStatus.open ? "#22c55e" : "#666", display: "inline-block" }} />
                {openStatus.text}
              </span>
            ) : cityLabel ? (
              <span style={{ background: "rgba(255,255,255,0.1)", backdropFilter: "blur(8px)", color: "#fff", borderRadius: 9999, padding: "5px 14px", fontSize: 12, fontWeight: 600, border: "1px solid rgba(255,255,255,0.15)" }}>📍 {cityLabel}</span>
            ) : null}
          </div>
          <h1 className="dk-name" style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 700, fontSize: "clamp(2.5rem, 9vw, 5.5rem)", color: "#fff", lineHeight: 1.0, letterSpacing: "0.04em", textTransform: "uppercase", marginBottom: 16 }}>{business.name}</h1>
          {business.tagline && <p className="dk-tag" style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 400, fontSize: "clamp(0.95rem, 2.5vw, 1.15rem)", color: accent, lineHeight: 1.5, letterSpacing: "0.08em", marginBottom: 20, textTransform: "uppercase" }}>{business.tagline}</p>}
          {igHandle && (
            <div className="dk-ig" style={{ marginBottom: 28, display: "flex", justifyContent: "center" }}>
              <a href={business.instagram_url ?? "#"} target="_blank" rel="noopener noreferrer"
                ...
                <IgIcon size={14} color={accent} />{igHandle}
              </a>
            </div>
          )}
          <button className="dk-cta" onClick={openFromCTA}
            style={{ fontFamily: "'Oswald', sans-serif", background: accent, border: "none", color: D.bg, padding: "14px 40px", borderRadius: 4, fontSize: 16, fontWeight: 700, cursor: "pointer", letterSpacing: "0.06em", textTransform: "uppercase", transition: "background 0.2s, color 0.2s" }}
      ...
      {(() => {
        const bar = (
          <div style={{ background: D.surface, padding: "16px 20px", textAlign: "center", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, borderBottom: `1px solid ${D.border}` }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 600, color: D.text }}>{t.social.happyClients}</span>
          </div>
        );
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "0 20px 140px" }}>
        ...
        <section ref={servicesRef} style={{ paddingTop: 60 }}>
          <h2 style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 700, fontSize: 26, letterSpacing: "0.06em", textTransform: "uppercase", color: accent, marginBottom: 28 }}>{t.services.title}</h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {services.map((s, i) => {
              const hovered = hoveredCard === s.id;
              return (
                <div key={s.id} onMouseEnter={() => setHoveredCard(s.id)} onMouseLeave={() => setHoveredCard(null)}
                  style={{ background: D.surface, border: `1px solid ${D.border}`, borderInlineStart: `3px solid ${hovered ? accent : "transparent"}`, borderRadius: 8, padding: "16px 18px", display: "flex", justifyContent: "space-between", alignItems: "center", opacity: servicesVisible ? 1 : 0, transform: servicesVisible ? "translateY(0)" : "translateY(18px)", transition: [`opacity 0.5s ease ${i*70}ms`, `transform 0.5s ease ${i*70}ms`, "border-color 0.2s"].join(", ") }}
                >
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 15, fontWeight: 600, color: D.text, marginBottom: 2 }}>{s.name}</div>
                    {s.description && <div style={{ fontSize: 13, color: D.muted, marginBottom: 4, lineHeight: 1.4 }}>{s.description}</div>}
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 14, flexShrink: 0, marginInlineStart: 12 }}>
                    ...
                    <button onClick={() => openFromService(s)}
                      style={{ fontFamily: "'Oswald', sans-serif", background: accent, color: D.bg, border: "none", borderRadius: 4, padding: "7px 16px", fontSize: 13, fontWeight: 700, cursor: "pointer", letterSpacing: "0.04em", transition: "background 0.2s", whiteSpace: "nowrap" }}
                      onMouseEnter={e=>{e.currentTarget.style.background="#fff";}} onMouseLeave={e=>{e.currentTarget.style.background=accent;}}>
                      {t.services.book}
                    </button>
                  </div>
                </div>
        ...
        {business.show_about !== false && business.about_text && (
          <section style={{ paddingTop: 60 }}>
            <h2 style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 700, fontSize: 26, letterSpacing: "0.06em", textTransform: "uppercase", color: accent, marginBottom: 24 }}>{t.about.title}</h2>
            <div style={{ background: D.surface, borderRadius: 10, padding: "24px 20px", border: `1px solid ${D.border}` }}>
              <div className="dk-about-row">
                ...
                <p style={{ fontSize: 15, lineHeight: 1.8, color: D.muted, margin: 0 }}>{business.about_text}</p>
              </div>
            </div>
          </section>
        )}
        ...
        {business.show_gallery !== false && business.gallery_images && business.gallery_images.length > 0 && (
          <section style={{ paddingTop: 60 }}>
            <h2 style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 700, fontSize: 26, letterSpacing: "0.06em", textTransform: "uppercase", color: accent, marginBottom: 20 }}>{t.gallery.title}</h2>
        ...
        {business.show_hours !== false && business.business_hours && (
          <section style={{ paddingTop: 60 }}>
            <h2 style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 700, fontSize: 26, letterSpacing: "0.06em", textTransform: "uppercase", color: accent, marginBottom: 20 }}>{t.hours.title}</h2>
        ...
        {business.show_location !== false && business.address && (
          <section style={{ paddingTop: 60 }}>
            <h2 style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 700, fontSize: 26, letterSpacing: "0.06em", textTransform: "uppercase", color: accent, marginBottom: 20 }}>{t.location.title}</h2>
        ...
        <footer style={{ marginTop: 64, textAlign: "center" }}>
          ...
          {(business.instagram_url || business.whatsapp_number || business.facebook_url) && (
            <div style={{ display: "flex", justifyContent: "center", gap: 10, marginBottom: 14 }}>
              {business.instagram_url && <a href={business.instagram_url} target="_blank" rel="noopener noreferrer" style={{ width: 36, height: 36, borderRadius: "50%", background: accent, display: "flex", alignItems: "center", justifyContent: "center" }} onMouseEnter={e=>{e.currentTarget.style.opacity="0.75";}} onMouseLeave={e=>{e.currentTarget.style.opacity="1";}}><IgIcon size={16} color={D.bg}/></a>}
              {business.whatsapp_number && <a href={`https://wa.me/${business.whatsapp_number.replace(/\D/g,"")}`} target="_blank" rel="noopener noreferrer" style={{ width: 36, height: 36, borderRadius: "50%", background: accent, display: "flex", alignItems: "center", justifyContent: "center" }} onMouseEnter={e=>{e.currentTarget.style.opacity="0.75";}} onMouseLeave={e=>{e.currentTarget.style.opacity="1";}}><WaIcon size={16} color={D.bg}/></a>}
              {business.facebook_url && <a href={business.facebook_url} target="_blank" rel="noopener noreferrer" style={{ width: 36, height: 36, borderRadius: "50%", background: accent, display: "flex", alignItems: "center", justifyContent: "center" }} onMouseEnter={e=>{e.currentTarget.style.opacity="0.75";}} onMouseLeave={e=>{e.currentTarget.style.opacity="1";}}><FbIcon size={16} color={D.bg}/></a>}
            </div>
          )}
          <div style={{ fontSize: 12, color: accent, fontWeight: 600 }}>{t.footer.poweredBy}{" "}<a href="https://bapita.com" style={{ color: accent, textDecoration: "none" }}>{t.footer.brand}</a></div>
        </footer>
      </div>
      ...
      {waNumber && (
        <a href={`https://wa.me/${waNumber}`} target="_blank" rel="noopener noreferrer"
          style={{ position: "fixed", bottom: 90, insetInlineStart: 20, width: 52, height: 52, borderRadius: "50%", background: accent, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 4px 20px ${accent}44`, zIndex: 50, transform: showWa ? "translateY(0) scale(1)" : "translateY(20px) scale(0.85)", opacity: showWa ? 1 : 0, transition: "transform 0.35s ease, opacity 0.35s ease", pointerEvents: showWa ? "auto" : "none" }}>
