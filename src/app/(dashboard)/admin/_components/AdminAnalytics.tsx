...
// re-checks the admin allowlist. Middleware already blocks /admin/* for
// non-admin emails.
...
// Parentheticals clarify the ambiguous buckets for a non-technical reader.
const SOURCE_HINT: Record<string, string> = {
  direct: "Direct (typed the address or opened a bookmark)",
  other: "Other (came from a website we could not identify)",
};

function labelWithHint(s: Source) {
  return SOURCE_HINT[s.key] || s.label;
}
    ...
    <div style={{ maxWidth: 960, margin: "0 auto", padding: "20px 24px 64px" }}>
      ...
      {loading || !data ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          {/* Source mix */}
          <Card title="Platform traffic sources" subtitle="First place each visitor arrived from">
            <Bars items={data.sourceMix.map((s) => ({ key: s.key, label: labelWithHint(s), count: s.count }))} total={data.totals.visitors} color="linear-gradient(90deg, #7BA6C9, #5D8AB0)" />
          </Card>

          {/* Leaderboard */}
          <Card title="Businesses" subtitle="Sorted by visitors. Zero-traffic clients flagged. No times = visitors who picked a date with no open slots.">
            ...
            {!selectedBiz ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                ...
                <div>
                  ...
                  <Bars items={selectedBiz.sources.map((s) => ({ key: s.key, label: labelWithHint(s), count: s.count }))} total={selectedBiz.visitors} color="linear-gradient(90deg, #7BA6C9, #5D8AB0)" />
                </div>
              </div>
            )}
          </Card>
